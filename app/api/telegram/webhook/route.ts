...
// ─── Main Logic (runs async after 200 response) ───────────────────────────────
    ...
    if (text === '/start') {
      ...
      if (!hasAccess && !isAdmin) {
        ...
        await sendMessage(chatId,
          ...
          `<i>Tap below to open the terminal.</i>`,
          {
            reply_markup: {
              inline_keyboard: [
      ...
      const statusLabel = isAdmin ? '🛡️ Admin' : (hasAccess ? '✅ Authorized' : '🔒 Restricted');
      await sendMessage(chatId,
      ...
      await sendMessage(chatId,
        `Use /start to open the terminal, or /help for commands.`,
        {
          reply_markup: {
            inline_keyboard: [[{ text: '🖥 Open Terminal', web_app: { url: APP_URL } }]]
          }
        }
      );
    }
...
// ─── Main Route ── RESPOND 200 IMMEDIATELY, process async ────────────────────
export async function POST(request: NextRequest) {
  if (!BOT_TOKEN) return NextResponse.json({ ok: true });

  // Validate secret token before parsing body
  ...
  // ⚡ CRITICAL: Respond 200 immediately to prevent Telegram 504 timeout on Netlify cold starts.
  // All processing happens in a background Promise that outlives the response.
